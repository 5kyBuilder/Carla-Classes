...
def generate_lidar_blueprint(blueprint_library):
    lidar_blueprint = blueprint_library.find("sensor.lidar.ray_cast_semantic")#write code here for lidar
    lidar_blueprint.set_attribute('channels', '24')#write code here
    lidar_blueprint.set_attribute('points_per_second', '56000')#write code here
    lidar_blueprint.set_attribute('rotation_frequency', '40')#write code here
    lidar_blueprint.set_attribute('range', '100')#write code here
...
def check_traffic_lights():
    if dropped_vehicle.is_at_traffic_light():
        ...
        if traffic_light.get_state() == carla.TrafficLightState.Red:
            ...
            dropped_vehicle.apply_control(carla.VehicleControl(handbrake=True))
    else:
...
def semantic_lidar_data(point_cloud):
    ...
    for detection in point_cloud:
        ...
        print("Distance: ", distance_name_data)

        distance_name_data["name"] = key_list[position]
        for object_name in object_id:
            if distance_name_data["name"] == object_name and distance_name_data["distance"] > 5 and distance_name_data["distance"] < 7:
                ...
                if distance_name_data["distance"] > 3 and distance_name_data["distance"] < 6:
                    ...
                    print(f"Object near the car is  {object_name} and distance is: {distance_name_data['distance']}")
                    break
            else:
                print(f"Object near the car is  {object_name} and distance is: {distance_name_data['distance']}")
try:
