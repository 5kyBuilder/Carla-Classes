...
def number_of_vehicle():
    ...
    if len(all_vehicles) > 1:
        ...
        for each_car in all_vehicles: #complete this code
            if each_car.id != world.id:
                get_distance_of_bot_vehicles.append((distance(each_car.get_location()), each_car)) #complete this code
        ...
        for distance_of_car, vehicle in sorted_vehicles: #complete this code
            if distance_of_car > 200: #complete this code
                break
            ...
            for distance_in_meter in final_vehicle_result: #complete this code
                if distance_in_meter["distance"] > 6 and distance_in_meter["distance"] < 10:
                    ...
                    time.sleep(3)
                    car_control()
                if distance_in_meter["distance"] > 1 and distance_in_meter["distance"] < 6:
                    print("Close car")
...
def car_control():
    ...
    dropped_vehicle.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Interior | carla.VehicleLightState.LowBeam))  #Turn on LowBeam light
    time.sleep(1)
    dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.4)) 
    dropped_vehicle.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Interior | carla.VehicleLightState.LowBeam)) #Turn on LowBeam light
    time.sleep(3)
    dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
    dropped_vehicle.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Interior | carla.VehicleLightState.LowBeam)) #Turn on Brake light
    time.sleep(5)
