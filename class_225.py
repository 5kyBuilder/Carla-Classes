...
def _Radar_callback(radar_data):
    ...
    for detection in radar_data:
        ...
        if distance_name_data["distance"] > 5 and distance_name_data["distance"] < 7:
            print("Slowing down the car")
            dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.31))
            if distance_name_data["distance"] > 1 and distance_name_data["distance"] < 6:
                dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
                print("Changed distance and name data: ", distance_name_data)
...
try:
    ...
    spawn_point = (world.get_map().get_spawn_points()[56])
