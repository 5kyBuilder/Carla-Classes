...
def lidar_data(point_cloud, lidar_point_cloud_buffer):
    matrix_representationa_data = np.copy(np.frombuffer(point_cloud.raw_data, dtype=('f4')))
    reshape_of_image = np.reshape(matrix_representationa_data, (int(matrix_representationa_data.shape[0] / 4), 4))
